...
<li>♻️ Eco-friendly lifestyle choices</li>
...
How can I help you today?`;
    ...
    function getBiggestSourceResponse(fp) {
        ...
        const categories = [
            ...
            { name: 'Diet & Food', value: fp.breakdown.diet.total, icon: '🍽️' },
            { name: 'Lifestyle', value: fp.breakdown.lifestyle.total, icon: '🛍️' },
...
${fp.total <= countryAvg ? '👏 Great job! You\'re already below your country\'s average!' : '💪 There\'s room for improvement. Every small change counts!'}`;
    ...
    function getTransportTips(fp) {
        ...
        if (fp && fp.breakdown.transport.flights > 0.5) {
            tips += `✈️ <strong>Flights are your big one!</strong> At ${fp.breakdown.transport.flights.toFixed(1)}t/yr, reducing just one flight saves ~0.55t.\n\n`;
        }
...
${fp ? `Your current transport footprint: <strong>${fp.breakdown.transport.total.toFixed(1)}t/year</strong>` : ''}`;
    ...
    function getEnergyTips(fp) {
        ...
        if (fp && fp.inputs.energySource === 'coal') {
            tips += `⚠️ <strong>You're on a high-carbon energy grid.</strong> Switching to renewable energy could save up to 1.5t/yr!\n\n`;
        }

        tips += `<ul>
<li>☀️ <strong>Switch to renewable energy</strong> — choose a green energy provider</li>
<li>💡 <strong>Use LED bulbs</strong> — they use 75% less energy than incandescent</li>
<li>🌡️ <strong>Adjust thermostat</strong> — 1°C lower in winter saves ~300 kg CO₂/yr</li>
...
${fp ? `Your current energy footprint: <strong>${fp.breakdown.energy.total.toFixed(1)}t/year</strong>` : ''}`;
    ...
    function getDietTips(fp) {
        ...
        return `🍽️ <strong>Sustainable Diet Tips:</strong>
...
💡 <strong>Fun fact:</strong> If everyone ate plant-based, global food emissions would drop by 70%!`;
    ...
    function getLifestyleTips(fp) {
        return `🛍️ <strong>Sustainable Lifestyle Tips:</strong>
...
<li>🏷️ <strong>Support sustainable brands</strong> — look for B-Corp and Fair Trade certifications</li>
</ul>
...
${fp ? `Your lifestyle footprint: <strong>${fp.breakdown.lifestyle.total.toFixed(1)}t/year</strong> (clothing: ${fp.breakdown.lifestyle.clothing.toFixed(2)}t, electronics: ${fp.breakdown.lifestyle.electronics.toFixed(2)}t)` : ''}`;
    ...
    function getRecyclingTips(fp) {
        return `♻️ <strong>Recycling & Waste Reduction:</strong>

<ul>
<li>♻️ <strong>Follow the 5 R's:</strong> Refuse → Reduce → Reuse → Recycle → Rot (compost)</li>
<li>🗑️ <strong>Sort waste properly</strong> — contaminated recycling goes to landfill</li>
...
<li>☀️ <strong>Renewable energy</strong> — funding solar/wind projects in developing countries</li>
<li>🔥 <strong>Methane capture</strong> — preventing methane from landfills entering the atmosphere</li>
<li>🏘️ <strong>Clean cookstoves</strong> — providing efficient stoves in developing nations</li>
</ul>

<strong>Important considerations:</strong>
<ul>
<li>⚠️ Offsets should be a <strong>last resort</strong> — reduce first, offset the rest</li>
...
🌍 <strong>Reduce first, offset second</strong> — every ton you prevent is better than one offset.`;
    ...
    function getChallengeResponse() {
        const challenges = [
            ...
            { emoji: '🛍️', text: 'Bring a reusable bag on your next shopping trip' },
...
Check out the <strong>Challenges</strong> tab for more daily and weekly eco-challenges with points and badges! 🎖️
...
<li>♻️ Recycle and compost regularly</li>
<li>🛍️ Use reusable bags and water bottles</li>
...
<ul>
<li>🌡️ Earth has warmed by <strong>1.1°C</strong> since pre-industrial times</li>
...
<li>🍽️ <strong>Food</strong> (agriculture, processing) — ~21%</li>
<li>🛍️ <strong>Goods & services</strong> (manufacturing, shipping) — ~21%</li>
</ul>
...
<li>🍽️ Diet: <strong>${fp.breakdown.diet.total.toFixed(2)}t</strong></li>
<li>🛍️ Lifestyle: <strong>${fp.breakdown.lifestyle.total.toFixed(2)}t</strong></li>
</ul>

📊 Compared to ${fp.comparison.country} average (${fp.comparison.countryAvg}t): <strong>${fp.total <= fp.comparison.countryAvg ? '✅ Below' : '⬆️ Above'} average</strong>
...
<li>🌧️ <strong>Collect rainwater</strong> — great for garden irrigation</li>
...
<li>🍽️ <strong>Run full loads</strong> — dishwashers and washing machines</li>
</ul>
...
<li>♻️ <strong>"Recycling tips"</strong> — Waste reduction strategies</li>
